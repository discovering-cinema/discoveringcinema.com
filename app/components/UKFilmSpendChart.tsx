...
const CustomTooltip = ({
  ...
  const notes: Record<string, string> = {
    '2018':
      '* Domestic spend anomaly — BFI attributed this partly to data collection methodology changes.',
    '2020': '* Production halted March–July due to Covid-19 lockdowns.',
    '2023':
      "* Hollywood writers' and actors' strikes disrupted inward investment productions.",
  };
  ...
  return (
    ...
    >
      <p style={{ margin: '0 0 8px', fontWeight: 600, color: '#fff' }}>
        {label}
      </p>
      {payload.map((entry) => (
...
export default function UKFilmSpendChart() {
  return (
    <figure style={{ margin: '2rem 0' }}>
      ...
      >
        † 2018 domestic spend (£319m) is an outlier. The BFI noted this partly
        reflected methodology changes in data collection. The 2020 dip reflects
        Covid-19 production shutdowns (March–July). The 2023 dip in inward
        investment reflects the Hollywood writers&apos; and actors&apos;
        strikes. All figures are film-only; high-end TV production is excluded.
      </p>
    </figure>
  );
}
