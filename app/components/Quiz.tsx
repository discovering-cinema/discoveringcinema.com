...
function ScoreMessage({ score, total }: { score: number; total: number }) {
  ...
  if (pct >= 0.8)
    return (
      <>
        Strong result — {score} out of {total}.
      </>
    );
  if (pct >= 0.6)
    return (
      <>
        {score} out of {total}. Not bad.
      </>
    );
  if (pct >= 0.4)
    return (
      <>
        {score} out of {total}. Room to go again.
      </>
    );
  return (
    <>
      {score} out of {total}. The article is still there.
    </>
  );
}
...
export default function Quiz({ questions, title }: QuizProps) {
  ...
  if (finished) {
    ...
    const barColor =
      pct === 1 ? 'bg-success' : pct >= 0.6 ? 'bg-primary' : 'bg-warning';
      ...
      <div className="my-8 rounded-2xl border border-border bg-card text-foreground overflow-hidden font-sans">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="flex items-end gap-3 mb-2">
            ...
            <span className="text-muted-foreground font-semibold text-lg leading-none mb-1">
              / {questions.length}
            </span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <span
                      className={`text-base font-black leading-none mt-0.5 flex-shrink-0 ${correct ? 'text-success' : 'text-destructive'}`}
                    >
                      {correct ? '✓' : '✕'}
                    </span>
                    <div>
                      ...
                      <p
                        className={`text-xs font-semibold ${correct ? 'text-success' : 'text-muted-foreground'}`}
                      >
                        {correct ? '' : 'Correct: '}
                        {q.options[q.correct]}
                      </p>
      ...
      <div className="p-6 md:p-8">
        ...
        >
          {q.options.map((option, idx) => {
            ...
            let optionStyle =
              'border-border bg-muted text-foreground hover:border-foreground/30 hover:bg-border cursor-pointer';

            if (isAnswered) {
              if (isRight) {
                optionStyle =
                  'border-success/60 bg-success/10 text-success cursor-default';
              } else if (isChosen) {
                optionStyle =
                  'border-destructive/60 bg-destructive/10 text-destructive cursor-default';
              } else {
                optionStyle =
            ...
            return (
              ...
              >
                <span
                  className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-black flex-shrink-0 transition-all ${
                    isAnswered && isRight
        ...
        {isAnswered && !q.explanation && (
          ...
          >
            {isCorrect
              ? 'Correct.'
              : `Incorrect. The answer was: ${q.options[q.correct]}`}
          </p>
        )}
